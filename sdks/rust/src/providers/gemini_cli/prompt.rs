...
/// Flatten multi-turn messages into `(system_prompt, user_prompt)` for
/// the Gemini CLI headless mode.
///
/// Gemini CLI has no `--system-prompt` flag, so callers merge `system`
/// into the stdin prompt themselves. Keeping the tuple signature matches
/// the sibling [`claude_code::prompt::messages_to_prompt`](
/// crate::providers::claude_code::prompt::messages_to_prompt) so future
/// shared helpers can operate on both providers uniformly.
