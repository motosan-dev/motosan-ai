...
- `tests/integration/test_anthropic_live.py::test_live_opus_4_8_adaptive_thinking` — **amendment 2026-07-17** (added after the PR-P executor's blocking report; the drafting sweep only covered the unit-gate scope `tests/` minus `tests/integration`, but the repo's pre-push hook also runs the live suite). Line 250's `assert any(event.event_type == "thinking" and event.content for event in events)` flips to `assert any(event.event_type == StreamEventType.thinking_delta and event.content for event in events)`; add `StreamEventType` to the file's existing `motosan_ai` import block. All other assertions unchanged — the test's intent (live adaptive thinking must produce thinking content) is preserved; post-F3 a `thinking_done` event also arrives, but the delta assertion is sufficient and tighter.
