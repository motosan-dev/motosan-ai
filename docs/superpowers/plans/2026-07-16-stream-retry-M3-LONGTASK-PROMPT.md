...
PROCEDURE — per task, ascending order (sequential within a group):
  ...
  3. Locked design (deviation = wrong even if it compiles): IncompleteStream (Rust) / IncompleteStreamError subclass-of-StreamError (Py/TS) / CancelledError (TS) / StreamReadTimeoutError (Py); adapter-level EOF enforcement; OpenAI-wire terminal (OpenAI + Python MiniMax) is `[DONE]` OR a finish_reason-bearing chunk — EITHER suffices (semantic completion counts; IncompleteStream fires only when NEITHER was seen — amended 2026-07-17, supersedes the earlier strict-[DONE] wording); timeouts connect 10s / read-idle 120s (streaming reads ONLY — blessed narrowing) / total None (chat-only); Rust build-once providers + shared reqwest client; TS caller-signal → CancelledError never retried, fetch-internal AbortError stays retryable; CancelledError conformance row is TS-ONLY.
  4. PINNED-BEHAVIOR FLIPS: M3 deliberately retires the v0.10.1 "exactly one done on truncated EOF" invariant. ONLY the tests named in your task's flip list may change; every other M1/M2 test (retry, conformance, stream guards) MUST pass unchanged. ONE carve-out — the plan's "Fixture-completion allowance": a test whose fixture merely OMITS the provider terminal event (assertions target unrelated behavior) may have the terminal event APPENDED, assertions otherwise unchanged; list each under "Fixture completions:" in the commit message; if any assertion beyond tolerating the terminal-derived done would need to change, STOP and report (that is flip-list territory). Editing any other test to make it pass remains unacceptable — stop and report instead.
